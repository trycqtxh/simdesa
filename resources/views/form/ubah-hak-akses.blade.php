    ...
    <div id="tab">
        ...
        <fieldset>
            ...
            <div class="form-group form-group-sm">
                ...
                <div class="col-md-9">
                    <input class="form-control" name="description" id="description" value="{{ $role->description }}">
                </div>
            </div>
            <div class="clearfix"></div>
        </fieldset>
            ...
            @foreach($master as $m)
                <fieldset class="fieldset-anak-4" id="{{ $m->display_name }}">
                    ...
                    @foreach($sub_master as $sm)
                        <label for="id_{{ $sm->name }}">
                            <input type="checkbox" id="id_{{ $sm->name.'-'.$sm->display_name }}" name="permission[]" value="{{ $sm->id }}" {{ ($role->hasPermission($sm->name)) ? 'checked' : '' }}> {{ $sm->tampilan }}
                        </label>
                    @endforeach
                </fieldset>
            @endforeach
