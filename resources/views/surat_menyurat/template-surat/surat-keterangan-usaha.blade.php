...
<div class="row setup-content" id="step-2">
    ...
    <div class="col-xs-12 text-center well" id="template-surat" style="height: inherit; padding-left: 160px; padding-right: 160px">
        ...
        <br>
        <br>
        <p>Yang bertanda tangan di bawah ini Kepala {{ $profil->find('des')['value'].' '.$profil->find('nama_desa')['value'] }} Kecamatan {{ $profil->find('kecamatan')['value'] }} Pemerintah Daerah {{ $profil->find('kab')['value'].' '.$profil->find('kota')['value'] }}.</p>
        ...
        <p>Bahwa orang tersebut adalah benar warga kami yang memiliki usaha <div class="form-group form-group-sm col-sm-5 pull-left"><input class="form-control" name="usaha" ></div></p>
        <p>
            yang berlokasi di {{ $profil->find('des')['value'].' '.$profil->find('desa')['value'].' Kecamatan'.$profil->find('kecamatan')['value'].' Kecamatan'.' '.$profil->find('kab')['value'].' '.$profil->find('kota')['value'] }}
        </p>

        <p>Demikian keterangan ini kami berikan, agar yang berkepentingan mengetahui adanya.</p>
