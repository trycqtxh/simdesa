...
<div class="row setup-content" id="step-2">
    ...
    <div class="col-xs-12 text-center well" id="template-surat" style="height: inherit; padding-left: 160px; padding-right: 160px">
        <h3><u>SURAT KETERANGAN TIDAK MAMPU / MISKIN</u></h3>
        ...
        <br>
        <br>
        <p>Yang bertanda tangan di bawah ini Kepala {{ $profil->find('kepala_desa')['value'].' '.$profil->find('des')['value'] }} Kecamatan {{ $profil->find('kecamatan')['value'] }} Pemerintah Daerah {{ $profil->find('kab')['value'].' '.$profil->find('kabupaten')['value'] }},  menerangkan dengan sesungguhnya, bahwa :</p>
        ...
        <table id="surat" style="font-size:12pt; text-align: left; font-family: 'Times New Roman'" align="center" width="90%">
            ...
            <tr>
                ...
                <td>:</td>
                <td><div class="form-group form-group-sm"><input class="form-control" name="status_perkawinan" ></div></td>
            </tr>
            <tr>
                <td>&nbsp;</td>
                <td>Pekerjaan</td>
        ...
        <p>Keterangan ini kami berikan kapadanya dengan berdasarkan sepengetahuan dan data yang ada pada kami bahwa orang tersebut di atas adalah benar warga kami yang pada saat ini keadaan ekonominya sangat lemah dan tidak mampu/miskin.</p>
        <p>Surat Keterangan ini kami berikan untuk memenuhi <div class="form-group form-group-sm"><input class="form-control" name="tujuan" ></div> </p>
        <p>Demikian, dimohon kepada pihak terkait kiranya dapat memberikan bantuan serta agar maklum</p>
...
<script type="text/javascript">
    ...
    $(function(){
        tabel.on('select', function(){
            ...
            $('#step-2').find('input[name=bin_binti]').val(data['ayah']+' / '+data['ibu']);
            $('#step-2').find('input[name=jenis_kelamin]').val(data['jenis_kelamin']);
            $('#step-2').find('input[name=status_perkawinan]').val(data['status_perkawinan']);
