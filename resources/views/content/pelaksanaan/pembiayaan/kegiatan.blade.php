...
@section('content-main')
    ...
    <div class="box box-default">
        ...
        <div class="box-body">
            <div class="btn-group btn-group-sm">
                ...
                @permission('add-pembiayaan-pelaksanaan')
                <button class="btn btn-default" id="btn-tambah"><i class="fa fa-plus"></i> Tambah</button>
                @endpermission
                @permission('edit-pembiayaan-pelaksanaan')
                <button class="btn btn-default" id="btn-ubah" disabled><i class="fa fa-edit"></i> Ubah</button>
                @endpermission
                @permission('remove-pembiayaan-pelaksanaan')
                <button class="btn btn-default" id="btn-hapus" disabled><i class="fa fa-trash"></i> Hapus</button>
                @endpermission
            </div>
    ...
    @include('modal.pelaksanaan.pembiayaan')
@endsection
...
<script type="text/javascript">
    ...
    var table = $('#tabel-realisasi').DataTable({
        ...
        ajax      : {
            ...
            url     : "{{ route('pelaksanaan.apbd.pembiayaan.kegiatan', $id) }}",
    ...
    $(function(){
        ...
        modalTambah.find('form').submit(function(e){
            ...
            $.ajax({
                ...
                dataType : "json",
                data     : $(this).serialize(),
                url      : "{{ route('pelaksanaan.apbd.pembiayaan.tambah') }}"
            }).done(function(){
        ...
        btnUbah.click(function(){
            ...
            $.ajax({
                global    : false,
                url       : "{{ route('pelaksanaan.apbd.pembiayaan.cari', '') }}/"+id,
                dataType  : "json"
            }).done(function(data){
                var data = data[0];
        ...
        modalUbah.find("form").submit(function(e){
            ...
            $.ajax({
                ...
                dataType: "json",
                data    : $(this).serialize(),
                url     : "{{ route('pelaksanaan.apbd.pembiayaan.ubah', '') }}/"+id,
            }).done(function(){
                $('input[name=jumlah]').inputmask({alias: "rupiah"});
                table.ajax.reload();
                info.ajax.reload();
        ...
        btnHapus.click(function(){
            ...
            if(confirm('Apakah Yakin Nomor Bukti "'+nomor+'" ini Akan Dihapus ?')){
                $.ajax({
                    ...
                    type    : "POST",
                    dataType: "json",
                    url      : "{{ route('pelaksanaan.apbd.pembiayaan.hapus', '') }}/"+id
                }).done(function(e){
                    table.ajax.reload();
                    info.ajax.reload();
                    control_panel();
